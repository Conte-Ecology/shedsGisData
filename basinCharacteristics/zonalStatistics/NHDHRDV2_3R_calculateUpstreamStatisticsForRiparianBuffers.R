...
for (catchmentsFileName in catchmentsFileNames) {
  ...
  # -----------------------
  # The delineation network needs to be matched to the hydro region of the current layer
  hydroID <- strsplit(catchmentsFileName, "_")[[1]][2]
  load(file.path(baseDirectory, "versions", outputName, "delineatedCatchments", paste0("Delineation_Catchments", hydroID, ".RData")))
